...
- 주문 / 결제:
  ...
  - [POST /order_details, {각각의 주문 상품 데이터}] 엔드포인트로 order의 아이디를 통해 order_detail 테이블에 레코드를 추가할 수 있도록 주문한 상품 데이터를 전달해줌.
...
- 마이페이지 - 주소관리:
  [GET /addresses/user?user_id={user_id}] 각 유저가 작성한 주소를 가져옴
...
- 관리자 - 주문 관리:
  ...
  - [GET /orderdetails/?order_dnb={order_dnb}] 해당 주문에 대한 상품 목록 가져오기
